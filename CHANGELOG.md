...
### Fixed
- An election in progress now survives a server restart. Elections, candidates and votes are written to `elections.json`, `candidates.json` and `voters.json` in `plugins/Democracy/` when the plugin is disabled and read back when it is enabled; previously every `save()` returned nothing and every `load()` was empty, so a restart silently discarded the election and all its votes. A file that cannot be read is reported in the console and left untouched — nothing is loaded from it and nothing is written over it on shutdown — so a corrupt file can be repaired rather than lost. State is written on shutdown only; a server that crashes rather than stopping still loses whatever changed since it last started
